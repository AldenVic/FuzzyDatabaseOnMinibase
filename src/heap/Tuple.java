...
public class Tuple implements GlobalConst{
	...
	/**
	 * The AttrTypes used in setHdr();
	 * null if setHdr is not called 
	 */
	private AttrType _types[];

	private short[] _strSizes;
	...
	public float getScore() throws IOException, FieldNumberOutOfBoundException {
		//return Convert.getFloValue((fldCnt + 2) * 2 + tuple_offset, data);
		return getFloFld(fldCnt);
	...
	/**
	 * set the score value of this tuple, this method assumes tht the header of the tuple
	 * was created using setHdr() 
	 * @throws FieldNumberOutOfBoundException 
	 * **/
	public void setScore(float score) throws IOException, FieldNumberOutOfBoundException {
		//Convert.setFloValue(score, (fldCnt + 2) * 2 + tuple_offset, data);
		//Convert.setFloValue(score, scoreIndex, data);
		setFloFld(fldCnt, score);
	}
	 ...
	 * TODO this function has been modified
	 * @throws FieldNumberOutOfBoundException 
	 *
	 */
	...
	public void setHdr (short numFlds,  AttrType types[], short strSizes[])
		...
		// note: we're adding score in the header as a float so 4 bytes
		...
		// incr for the last field is always 4, because score is float
		fldOffset[fldCnt]  = (short) (fldOffset[fldCnt-1] + 4);
		Convert.setShortValue(fldOffset[fldCnt], pos, data);

		// set the default value for the score
		// TODO removed this because, the iterators were overwriting setHdr on an already initialized tuple
//		if (_firstCall) {
//			_firstCall = false;
...
//		}
	...
	public String toString() {
		...
		for (int i = 0; i < _types.length; i++) {
			switch (_types[i].attrType) {
			case AttrType.attrString:
				try {
					output += getStrFld(i + 1);
				} catch (Exception e) {
					e.printStackTrace();
				}
				break;
	...
	public int hashCode() {
		...
		for (int i = 0; i < _types.length; i++) {
			...
			case AttrType.attrString:
				try {
					result = prime * result + getStrFld(i + 1).hashCode();
				} catch (Exception e) {
					e.printStackTrace();
				}
				break;
			case AttrType.attrInteger:
				try {
					result = prime * result + getIntFld(i + 1);
				} catch (Exception e) {
					e.printStackTrace();
				} 
				break;
			case AttrType.attrReal:
				try {
					result = prime * result + Float.floatToIntBits(getFloFld(i + 1));
				} catch (Exception e) {
					e.printStackTrace();
				}
				break;
	...
	public Tuple getCopy() throws InvalidTypeException, InvalidTupleSizeException,
		FieldNumberOutOfBoundException, IOException {
		Tuple tupleCopy = new Tuple();
		tupleCopy.setHdr((short) _types.length, _types, _strSizes);
		tupleCopy.tupleCopy(this);
		return tupleCopy;
	}
