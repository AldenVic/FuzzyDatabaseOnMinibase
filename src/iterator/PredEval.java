...
public class PredEval {
	...
	public static boolean Eval(CondExpr p[], Tuple[] tuples, AttrType[][] types)
			throws UnknowAttrType, InvalidTupleSizeException, InvalidTypeException,
			FieldNumberOutOfBoundException, PredEvalException, IOException {
		if (p == null) {
			return true;
		}
		for (int i = 0; i < p.length - 1; i++) {
			CondExpr condition = p[i];

			Tuple tuple1 = null;
			Tuple tuple2 = null;
			AttrType[] tuple1Types = null;
			AttrType[] tuple2Types = null;

			CondExpr[] newCondition = new CondExpr[2];
			newCondition[1] = null;
			newCondition[0] = new CondExpr();
			newCondition[0].type1 = condition.type1;
			newCondition[0].type2 = condition.type2;
			newCondition[0].op = condition.op;

			if (condition.type1.attrType == AttrType.attrSymbol) {
				FldSpec op1 = condition.operand1.symbol;
				tuple1 = tuples[op1.table];
				tuple1Types = types[op1.table];
				newCondition[0].operand1 = new Operand();
				newCondition[0].operand1.symbol = new FldSpec(new RelSpec(RelSpec.outer), op1.offset);
			} else {
				newCondition[0].operand1 = condition.operand1;
			}

			if (condition.type2.attrType == AttrType.attrSymbol) {
				FldSpec op2 = condition.operand2.symbol;
				tuple2 = tuples[op2.table];
				tuple2Types = types[op2.table];
				newCondition[0].operand2 = new Operand();
				newCondition[0].operand2.symbol = new FldSpec(new RelSpec(RelSpec.innerRel), op2.offset);
			} else {
				newCondition[0].operand2 = condition.operand2;
			}

			if (!Eval(newCondition, tuple1, tuple2, tuple1Types, tuple2Types)) {
				return false;
			}
		}
		return true;
