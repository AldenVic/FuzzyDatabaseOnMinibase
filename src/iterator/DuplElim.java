		  ...
		  )throws IOException ,DuplElimException
 {
		...
		if (!inp_sorted) {
			...
			for (int i = 0; i < in.length; i++) {
				try {
					sort2 = new Sort(in, len_in, s_sizes, sort1, in.length - i, order, sortFldLen, amt_of_mem);
			...
			_am = sort2;
//			try {
//				_am = new Sort(in, len_in, s_sizes, am, 1, order, sortFldLen,
//						amt_of_mem);
//			} catch (SortException e) {
//				e.printStackTrace();
//				throw new DuplElimException(e,
//						"SortException is caught by DuplElim.java");
//			}
		}
		...
		try {
			...
			// TODO do an if check for when the list is empty
			TempTuple1.tupleCopy(_am.get_next());
		} catch (Exception e) {
			throw new DuplElimException(e, "setHdr() failed");
		}
		done = false;
	   ...
	   Exception
 {
		...
		float scoreSum = TempTuple1.getScore();
		...
		do {
			...
			if (TupleUtils.Equal(TempTuple1, TempTuple2, _in, in_len)) {
				duplicateCount++;
				scoreSum += TempTuple2.getScore();
			}//
		...
		Jtuple.setScore(scoreSum/duplicateCount);
		return Jtuple;
	}
