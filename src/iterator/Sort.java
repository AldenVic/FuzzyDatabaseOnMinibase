...
public class Sort extends Iterator implements GlobalConst
{
  ...
  private boolean _scoreSort = false;
	   ...
	   Exception
  {
    ...
    for (i=0; i<n_R_runs; i++) {
      ...
      try {
    	  if (_scoreSort) {
    		  AttrType[] reducedTypes = new AttrType[_in.length - 1];
    		  for (int j = 0; j < reducedTypes.length; j++) {
				reducedTypes[j] = _in[j];
    		  }
    		  temp_tuple.setHdr((short)(n_cols - 1), reducedTypes, str_lens);
    	  } else {
	   ...
	   Exception
  {
    ...
    try {
    	if (_scoreSort) {
    		...
    		lastElem.setHdr((short)(n_cols-1), reducedTypes, str_lens);
    	} else {
    ...
    if (i_buf[cur_node.run_num].empty() != true) { 
      ...
      try {
    	  ...
    	  if (_scoreSort) {
    		  ...
    		  new_tuple.setHdr((short)(n_cols - 1), reducedTypes, str_lens);
    	  } else {
	      ...
	      ) throws IOException, SortException
  {
    ...
    try {
    	if (_scoreSort) {
    		...
    		t.setHdr((short)(len_in - 1), reducedTypes, str_sizes);
    	} else {
    ...
    try {
    	if (_scoreSort) {
    		...
    		op_buf.setHdr((short)(n_cols - 1), reducedTypes, str_lens);
    	} else {
