...
public class TupleUtils
{
	...
	public static boolean Equal(Tuple t1, Tuple t2, AttrType types[], int len)
		...
		// TODO don't compare the last field because its always score; see Tuple#setHdr
		// check to see if len being passed includes score or not
		// do a fool proof check for when, in case, they do use the internal field count
		// to perform equal
		for (i = 1; i <= len; i++)
	...
	public static short[] setup_op_tuple(Tuple Jtuple, AttrType[] res_attrs,
		...
		short[] res_str_sizes = getOPAttrInfo(res_attrs, in1, len_in1, in2,
				len_in2, t1_str_sizes, t2_str_sizes, proj_list, nOutFlds);
		try {
	...
	/**
	 * Sets the header of output tuple for a multi-join
	 * 
	 * @param jTuple The reference to the output tuple
	 * @param resAttrs The reference to the output attribute types
	 * @param inputTypes The input types of all the tables
	 * @param inputStrSizes The input string sizes of all the tables
	 * @param projList The projection list
	 * @return The output string sizes
	 */
	public static short[] setup_op_tuple(Tuple jTuple, AttrType[] resAttrs, AttrType[][] inputTypes,
			short[][] inputStrSizes, FldSpec projList[]) {
		// pick types from different tables to create output attrTypes list
		short[][] inputStrSizesIndexed = new short[inputTypes.length][];
		for (int i = 0; i < inputTypes.length; i++) {
			inputStrSizesIndexed[i] = new short[inputTypes[i].length];
			int count = 0;
			for (int j = 0; j < inputTypes[i].length; j++) {
				if (inputTypes[i][j].attrType == AttrType.attrString) {
					inputStrSizesIndexed[i][j] = inputStrSizes[i][count++];
				}
			}
		}
		int stringCount = 0;
		for (int i = 0; i < projList.length; i++) {
			int tableIndex = projList[i].table;
			int offset = projList[i].offset - 1; // offset starts from 1
			if (inputTypes[tableIndex][offset].attrType == AttrType.attrString) {
				stringCount++;
			}
		}
		short[] outputStrSizes = new short[stringCount];
		stringCount = 0;
		for (int i = 0; i < projList.length; i++) {
			int tableIndex = projList[i].table;
			int offset = projList[i].offset - 1; // offset starts from 1
			resAttrs[i] = new AttrType(inputTypes[tableIndex][offset].attrType);
			if (inputTypes[tableIndex][offset].attrType == AttrType.attrString) {
				outputStrSizes[stringCount++] = inputStrSizesIndexed[tableIndex][offset];
			}
		}
		try {
			jTuple.setHdr((short) resAttrs.length, resAttrs, outputStrSizes);
		} catch (Exception e) {
			System.err.println("error setting header of multi-join output tuple");
			e.printStackTrace();
		}
		return outputStrSizes;
	}

	public static short[] getOPAttrInfo(AttrType[] res_attrs, AttrType in1[], int len_in1,
			AttrType in2[], int len_in2, short t1_str_sizes[], short t2_str_sizes[],
